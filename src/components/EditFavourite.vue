...
export default {
	data() {
		return {
			req: this.$store.state.EditFav.req,
			name: this.$store.state.EditFav.name,
			sortBy: this.$store.state.EditFav.sortBy,
			maxQuontity: this.$store.state.EditFav.maxQuontity,
			isModalVisible: true,
		};
	},
	name: 'modal',
	mounted() {
		console.log('1234');
		// this.req = this.$store.state.EditFav.req;
	},
	methods: {
		save() {
			if (this.name) {
